...
F2=${2:-tests/pepe.fas}
F1=${3:-tests/ghcaller.test.5inds.8X.ori.fas}

echo $F1 $F2 
if [ ! -z $1 ]; then
	cat $F1  | fold -w $WIDTH | head -n $1 > ori
	cat $F2  | fold -w $WIDTH | head -n $1 > p
else
	cat $F1  | fold -w $WIDTH > ori
	cat $F2  | fold -w $WIDTH > p
fi

diff -s -y -W $(( 2*WIDTH )) ori p
